...
pub(crate) struct BitFieldQuadTree {
    ...
    //pub(crate) node_cost: Option<usize>,  // Cost of this node's encoding
}
...
pub(crate) struct QuadTree {
    ...
    pub(crate) node_cost: Option<usize>,
}
...
impl QuadTree {
    ...
    */
    ...
    pub(crate) fn divide(&mut self, data: &CsMat<u16>) {
        ...
        while let Some(node) = stack.pop() {
            ...
            let current_expense = encode_subarray(&node.points, data).map_or(0, |x| x.bytes());
            ...
            let mut nw_with_cost = nw;
            let mut ne_with_cost = ne;
            let mut se_with_cost = se;
            let mut sw_with_cost = sw;
             
            nw_with_cost.node_cost = Some(nw_expense);
            ne_with_cost.node_cost = Some(ne_expense);
            se_with_cost.node_cost = Some(se_expense);
            sw_with_cost.node_cost = Some(sw_expense);
             
             // Convert BitFieldQuadTree back to QuadTree and assign children
            node.nw = (!nw_with_cost.points.is_empty()).then_some(Box::new(nw_with_cost));
            node.ne = (!ne_with_cost.points.is_empty()).then_some(Box::new(ne_with_cost));
            node.se = (!se_with_cost.points.is_empty()).then_some(Box::new(se_with_cost));
            node.sw = (!sw_with_cost.points.is_empty()).then_some(Box::new(sw_with_cost));
    ...
    pub(crate) fn compare_parent_vs_children_expenses(
        ...
        pub(crate) fn calculate_optimal_expense(&mut self, node_id: &str, data: &CsMat<u16>) -> usize {
            if !self.divided {
            ...
            if let Some(ref mut nw) = self.nw {
                children_expense += nw.calculate_optimal_expense(&format!("{}_nw", node_id), data);
            }
            if let Some(ref mut ne) = self.ne {
                children_expense += ne.calculate_optimal_expense(&format!("{}_ne", node_id), data);
            }
            if let Some(ref mut se) = self.se {
                children_expense += se.calculate_optimal_expense(&format!("{}_se", node_id), data);
            }
            if let Some(ref mut sw) = self.sw {
                children_expense += sw.calculate_optimal_expense(&format!("{}_sw", node_id), data);
            }
    ...
    pub(crate) fn compute_quadtree_bit_fields(&self, data: &CsMat<u16>) -> BitFieldQuadTree {
        ...
        */
        ...
        if self.divided {
            ...
            // Copy cost from QuadTree to BitFieldQuadTree for internal nodes
            //node.node_cost = self.node_cost;
        } else {
            node.encoded_diffs = encode_subarray(&self.points, data).expect("expect nonempty");
            node.positions = self.positions.clone();
            //node.node_cost = Some(node.encoded_diffs.bytes());
        }
        node
    }
}
